...
for noise in noise_types:
    for i in d_values:
        for j in downscale_factors:
            for k in alphas:
                directory = cwd + "\do_wykresow_magisterka_factors\d_" + str(i) + "_scale_" + str(j) + "a_" + str(k)
                if noise != "symmetric":
                    directory = cwd + "\do_wykresow_magisterka_factors\d_" + str(i) + "_scale_" + str(j) + "noise_change_a"+str(k)
                pth = Path(directory)
